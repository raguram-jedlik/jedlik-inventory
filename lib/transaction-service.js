...
import { buildEmployeeOpenHoldings, validateReturnRequests } from './returns-service.mjs';
...
export async function processTransaction(data) {
  ...
  try {
    ...
    // For Returns, pre-validate against the employee's open holdings so we
    // never credit stock the employee never took. Computed once per request
    // and reused across the loop.
    let returnGuard = null;
    if (data.action === 'Return') {
      const allTxns = await getAllTransactions();
      const holdings = buildEmployeeOpenHoldings(data.employeeCode, allTxns);
      returnGuard = validateReturnRequests(data.items, holdings);
    }
    ...
    for (const txnItem of data.items) {
      ...
      // Return guard: refuse to credit stock the employee doesn't hold.
      // Per-line rejection preserves partial-success behavior.
      if (data.action === 'Return' && returnGuard && returnGuard[itemId] && !returnGuard[itemId].valid) {
        results.push({
          itemId,
          success: false,
          message: returnGuard[itemId].message,
        });
        continue;
      }
