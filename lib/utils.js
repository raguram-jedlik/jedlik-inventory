 ...
 *
 * Pure string work rather than createTextNode/innerHTML: callers build markup
 * during render (the scan page's confirm-modal body), and /scan is prerendered
 * on the server where `document` does not exist. Also escapes quotes, so the
 * result is safe inside an attribute as well as in text content.
 */
const HTML_ESCAPES = {
  '&': '&amp;',
  '<': '&lt;',
